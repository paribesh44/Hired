...
async def createApplyProfile(schema: save_job.SaveJob, db: Session = Depends(database.get_db), current_user: user.User = Depends(oauth2.get_user_job_seeker)):
...
def update_visibility(job_post_id:int, db: Session = Depends(database.get_db), current_user: user.User = Depends(oauth2.get_user_job_seeker)):
    update_saveJob = db.query(save_job.SaveJob).filter(
        save_job.SaveJob.job_post_id == job_post_id).first()
...
def all(db: Session = Depends(database.get_db), current_user: user.User = Depends(oauth2.get_user_job_seeker)):
    hired_saveJob = db.query(save_job.SaveJob).all()
