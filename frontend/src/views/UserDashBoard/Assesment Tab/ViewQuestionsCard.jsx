...
function QuestionCard(props) {
  ...
  const message = async () => {
    let mcq_response_obj = await callAPI({
      endpoint: `/mcq/get_mcq/${props.target_field_id}/${props.user_assesment_id}`,
    });
        ...
        {hired_mcq.data.answer_dict.map((val, key) => {
          return (
            <div key={key} className="questioncardmain">
              <div className="questionnumber">Question {val.id + 1}/20</div>
              <div className="question">Q. {val.question}?</div>

              <div className="questionsoptions">
                <div>
                  {val.options.map((val1, key2) => {
                    return (
                      <div key={key2}>
                        <div className="eachoption22">
                          {val.chosen_answer != val1.id
                            ? ""
                            : [
                                val1.isCorrect ? (
                                  <TiTick className="green_icon" />
                                ) : (
                                  <ImCross className="red_icon" />
                                ),
                              ]}
                          {val.chosen_answer != val1.id ? (
                            val1.isCorrect ? (
