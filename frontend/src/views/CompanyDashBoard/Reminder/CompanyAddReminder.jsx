...
function CompanyAddReminder() {
  ...
  const message = async () => {
    let response_obj = await callAPI({ endpoint: "/remainder/show_all_job_post_company" });
  ...
  async function handleChangeJobPost(e) {
    ...
    if (e.target.value != ""){
      let response_obj = await callAPI({ endpoint: `/remainder/get_seeker_applied_job/${e.target.value}` });
      ...
      <Grid container direction="column" className="reminder_page">
        ...
        <Grid item>
          <Grid container direction={"column"} className="add_a_new_reminder">
            <Grid item className="reminder_topic" style={{marginBottom: "5px"}}>
              Set Reminder Name:
            </Grid>
            <Grid item style={{marginBottom: "10px"}}>
              <input type="text" onChange={(e)=>{setRemainderName(e.target.value)}} placeholder={"e.g. Virtual Interview"}></input>
            </Grid>
            ...
            <Grid item style={{marginBottom: "10px"}}>
              <input type="text" onChange={(e)=>{setRemainderNote(e.target.value)}} placeholder={"e.g. Talk about his/her projects"}></input>
            </Grid>
            ...
            style={{marginBottom: "10px"}}
                ...
                >
                    ...
                    {jobPostOfCurrentCompany != null &&
                      jobPostOfCurrentCompany.map((i)=>( <MenuItem value={i.id}>{i.job} #{i.id}</MenuItem>))
                    }
                </Select>
            </Grid>
            ...
            <Grid container
              style={{marginBottom: "10px"}}
                ...
                onChange={(e)=>{setApplicantDropDown(e.target.value)}}
                displayEmpty
                inputProps={{ 'aria-label': 'Without label' }}
                >
                    <MenuItem value="">
                    ...
                    {appicant != null &&
                      appicant.map((i)=>( <MenuItem value={i.seeker.id}>{i.seeker.name} #{i.seeker.id} {i.description}</MenuItem>))
                    }
                </Select>
            </Grid>
            ...
            <Grid item>
              <input type="text" style={{ height: "40px" }} onChange={(e)=>{setMeetingLink(e.target.value)}} placeholder={"e.g. https://meet.google.com/xxxx"} />
            </Grid>
              ...
              <Grid container direction="row">
                <Grid item className="date_picker_item">
                  <Grid container direction="column">
                    <Grid item className="reminder_topic" style={{marginBottom: "8px"}}>
                      Set Day:
                    </Grid>
                ...
                <Grid item className="date_picker_item" style={{marginLeft: "40px"}}>
                  <Grid container direction="column">
                    <Grid item className="reminder_topic" style={{marginBottom: "8px"}}>
                      Set Time:
                    </Grid>
